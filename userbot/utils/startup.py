...
LOGS = logging.getLogger("دايفل العرب \n ")
...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://telegra.ph/file/3614f76c669d1a0a5f97f.jpg", caption="♛ ⦙ تـمّ اعـادة تشـغيل\n دايفل العـرب ✓  :  [ 7.5 ] .\n\n♛ ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n♛ ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n♛ ⦙ القناة الرسمية دايفل العرب : @R125R",                buttons=[(Button.url("مطور  دايفل الرسمي", "https://t.me/R125R"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(            "مجموعه بوت دايفل الخاص بك", iqthon, Config.TG_BOT_USERNAME, descript        )
