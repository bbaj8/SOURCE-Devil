...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**Catuserbot"):
            ...
            result = builder.article(
                title="Jmthon Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب دايفل العرب **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/R125R"),
                Button.url(
                    "المطور",
                    "t.me/KFFF6",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل دايفل من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[دايفل 🧸♥](https://t.me/R125R)",
