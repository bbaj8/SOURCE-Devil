...
async def iq(iqthonevent):
    ...
    IQTHON_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/3614f76c669d1a0a5f97f.jpg"
...
async def amireallyalive(event):
    ...
    IQTHON_ALIVE_TEXT = "❬ دايـفل العـرب - Telethon-Arabe ، 🕸  ❭ :"
...
async def _(event):
    ...
    OUTPUT = (f"☭︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس دايفل العرب @R125R:**\n\n{o}")
...
async def _(event):
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**☭︙   تم تحديث السورس ↻**")
    sandy = await edit_or_reply(event , "☭︙  جـاري تـحديـث   \n🔹 - قـد يستغـرق الأمـر 5 - 10 دقائـق انتـظـر\nلاتقـم بتحـديث أكثـر من 3 مـرات باليـوم ملاحضه(اذا توقف ولم يشتغل قم اعادة تشغيل يدوي تم نشر هنا https://t.me/telthon/44 )" ,)
    try:
...
async def psu(event):
    ...
    cpufreq = psutil.cpu_freq()
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
        cpuu = "**حجم استخدامك لدايفل العرب :**\n"
